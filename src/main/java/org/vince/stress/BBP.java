...
public class BBP {
    ...
    private static final Logger log = Logger.getLogger(BBP.class);

    /* Progress update vars */
    private volatile int prog;
    private int total;
    private volatile boolean run;
    private volatile BigDecimal tn = BigDecimal.ONE;
    long t0 = System.nanoTime();

    public String getInfo() {
        long dt = System.nanoTime() - t0;
        return "Elapsed time:" + dt + "ns\tTerm " + prog + "/" + total + " (" + (int) (((double) prog / (double) total) * 100) + "%)";
    }
    ...
    public BigDecimal calcPi(MathContext mc) {
        ...
        var pi = BigDecimal.ZERO.setScale(mc.getPrecision(), mc.getRoundingMode());
